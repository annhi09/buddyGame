...
document.addEventListener('DOMContentLoaded', () => {
  ...
  if(!hud || hud.dataset.ready === "1") return;
  ...
  if(parentBtn){
    parentBtn.addEventListener("contextmenu", e => e.preventDefault(), { capture:true });
    parentBtn.addEventListener("pointerdown", e => { try{ e.preventDefault(); }catch(_){} }, { passive:false });

    parentBtn.addEventListener("click", () => {
      sbOpenParent();

      const onLessonPage = !document.getElementById("lesson-page")?.classList.contains("hidden");
      if(onLessonPage && typeof handleParentClick === "function"){
        handleParentClick();
      }
    });
  }
...
async function sbUpdateHud(){
  ...
  starsEl.textContent = `⭐ ${prog.stars || 0}`;
  streakEl.textContent = `🔥 ${prog.streak || 0}`;
}
      ...
      let SB_READING_PARENT_UNLOCKED = false;
...
function handleParentClick(){
  if (SB_READING_PARENT_UNLOCKED) {
    SB_READING_PARENT_UNLOCKED = false;
    sbApplyReadingLessonLocks();
    sbToast("Lesson controls locked");
    return;
  }
  ...
  if (ans !== null && parseInt(ans, 10) === (a + b)) {
    SB_READING_PARENT_UNLOCKED = true;
    sbApplyReadingLessonLocks();
    sbToast("Lesson controls unlocked");
  } else if (ans !== null) {
    sbToast("Wrong answer");
  }
}

function sbApplyReadingLessonLocks(){
  const editBtn = document.getElementById("btn-edit");
  const delBtn = document.getElementById("btn-delete");

  if(editBtn) editBtn.style.display = SB_READING_PARENT_UNLOCKED ? "" : "none";
  if(delBtn) delBtn.style.display = SB_READING_PARENT_UNLOCKED ? "" : "none";
  const menu = document.getElementById('menu-page');
        if(menu){
          menu.querySelectorAll('button').forEach(b => {
            const txt = (b.textContent||'').toLowerCase();
            if(txt.includes('create') || txt.includes('export') || txt.includes('import') || txt.includes('reset') || txt.includes('edit')  || txt.includes('delete')){
      ...
      function sbApplyParentMode(){
        ...
        // Check if this is an "Open" button; if so, never hide it
      ...
      /* =========================
         Progress Helpers
      ========================= */
      const SB_READING_GAMELOCK_KEY = "sb_reading_gamelocks_v1";

function sbLoadReadingGameLocks(){
  try{
    return JSON.parse(localStorage.getItem(SB_READING_GAMELOCK_KEY) || "{}");
  }catch{
    return {};
  }
}

function sbSaveReadingGameLocks(map){
  localStorage.setItem(SB_READING_GAMELOCK_KEY, JSON.stringify(map));
}

function sbReadingLessonKey(lesson){
  return (lesson && (lesson.id || lesson.title))
    ? String(lesson.id || lesson.title)
    : "__reading_lesson__";
}

function sbIsReadingGameLocked(lessonKey, gameId){
  const map = sbLoadReadingGameLocks();
  return !!(map[lessonKey] && map[lessonKey][gameId]);
}

function sbSetReadingGameLocked(lessonKey, gameId, locked){
  const map = sbLoadReadingGameLocks();
  if(!map[lessonKey]) map[lessonKey] = {};
  map[lessonKey][gameId] = !!locked;
  sbSaveReadingGameLocks(map);
}
      ...
      async function sbConsumePendingImportReading(){
  try{
    const raw = localStorage.getItem("sb_pending_import_reading");
    if(!raw) return;

    const lesson = JSON.parse(raw);
    localStorage.removeItem("sb_pending_import_reading");
    ...
    sbToast("✅ Imported pack to Reading");
  }catch(err){
    console.warn("Reading import failed", err);
  }
}
      ...
      async function renderMenu(){
        ...
        for(const lesson of lessons){
          ...
          const firstImage = lesson.items?.[0]?.image || "";
          ...
          div.innerHTML = `
            <div class="lesson-thumb">
              ${
                firstImage
                  ? `<img class="lesson-thumb-img" data-img-src="${String(firstImage).replace(/"/g, '&quot;')}" alt="${String(lesson.title || 'Lesson').replace(/"/g, '&quot;')} cover" />`
            ...
            <div class="lesson-left">
              ...
              <div class="badges">
                ...
                ${isImportedPack ? `<span class="badge">🛍️ Marketplace Pack</span>` : ``}
                ${isImportedPack ? `<span class="badge">${sbFormatExportPolicy(policy.exportPolicy)}</span>` : ``}
              </div>
            </div>

            <div class="lesson-actions">
              <button style="background:#4ecdc4" ${locked ? 'disabled' : ''} data-open="${lesson.id}">Open</button>
              <button style="background:#6c5ce7" data-parent-only="1" data-export="${lesson.id}">⬇ Export</button>
              <button style="background:#3498db" data-parent-only="1" ${locked ? 'disabled' : ''} data-edit="${lesson.id}">Edit</button>
              <button style="background:#e74c3c" data-parent-only="1" data-del="${lesson.id}">Delete</button>
            </div>
          `;

          list.appendChild(div);

          const imgEl = div.querySelector('.lesson-thumb-img');
          if(imgEl){
            const rawSrc = imgEl.dataset.imgSrc || "";
            const resolvedSrc = await sbResolveImageSrc(rawSrc);
            ...
            if(resolvedSrc){
              ...
              div.querySelector('.lesson-thumb')?.appendChild(fallback);
            }
          }
        }

        list.querySelectorAll('[data-open]').forEach(b => {
          b.onclick = () => openLessonHubById(b.dataset.open);
        });

        list.querySelectorAll('[data-edit]').forEach(b => {
          b.onclick = () => editLessonById(b.dataset.edit);
        });

        list.querySelectorAll('[data-del]').forEach(b => {
          b.onclick = () => deleteLessonById(b.dataset.del);
        });

        list.querySelectorAll('[data-export]').forEach(b => {
          b.onclick = () => sbExportLessonById(b.dataset.export);
        });
      ...
      async function openLessonHubById(id){
        ...
        if(exportBtn){
          exportBtn.onclick = () => sbExportLessonById(currentLessonId);
          exportBtn.style.display = sbCanExportLesson(currentLesson) ? '' : 'none';
        }
        const prog = await sbGetProgress(currentLessonId) || sbDefaultProgress(currentLessonId);
        const count = (currentLesson.items || []).length;

        const badgesEl = document.getElementById('lesson-page-badges');
        if(badgesEl){
          badgesEl.innerHTML = `
            ...
            ${isImportedPack ? '<span class="badge">🛍️ Marketplace Pack</span>' : ''}
            ${isImportedPack ? `<span class="badge">${sbFormatAccessMode(policy.accessMode)}</span>` : ''}
            ${isImportedPack ? `<span class="badge">${sbFormatLicenseType(policy.licenseType)}</span>` : ''}
            ${isImportedPack ? `<span class="badge">${sbFormatExportPolicy(policy.exportPolicy)}</span>` : ''}
          `;
        }
        ...
        // if(badgesEl && isImportedPack){
        //   badgesEl.innerHTML += `
        //     <span class="badge">🛍️ Marketplace Pack</span>
        //     <span class="badge">${sbFormatAccessMode(policy.accessMode)}</span>
        //     <span class="badge">${sbFormatLicenseType(policy.licenseType)}</span>
        //     <span class="badge">${sbFormatExportPolicy(policy.exportPolicy)}</span>
        //   `;
        // }

        const completed = !!prog.completed;
        const lessonKey = sbReadingLessonKey(currentLesson);


        function sbBlockedReason(gameId){
        if(gameId !== "read" && !completed) return '📖 Finish "Read" mode to unlock games!';
        if(sbIsReadingGameLocked(lessonKey, gameId) && !SB_READING_PARENT_UNLOCKED) return "Locked by parent";
        return "";
      }

      function sbCanPlay(gameId){
        if(gameId === "read") return !sbIsReadingGameLocked(lessonKey, gameId) || SB_READING_PARENT_UNLOCKED;
        return completed && (!sbIsReadingGameLocked(lessonKey, gameId) || SB_READING_PARENT_UNLOCKED);
      }
        ...
        document.getElementById('btn-read').onclick =
          () => sbGuard("read", () => startRead(currentLessonIndex));

        document.getElementById('btn-smash').onclick =
          () => sbGuard("smash", () => startSmash(currentLessonIndex));

        document.getElementById('btn-scramble').onclick =
          () => sbGuard("scramble", () => startScrambleGame(currentLessonIndex));

        document.getElementById('btn-flash').onclick =
          () => sbGuard("flash", () => startFlashGame(currentLessonIndex));

        document.getElementById('btn-sentence').onclick =
          () => sbGuard("sentence", () => startSentenceRepeat(currentLessonIndex));

        document.getElementById('btn-daily').onclick =
          () => sbGuard("daily", () => startDailySpeaking(currentLessonIndex));

        const editBtn = document.getElementById('btn-edit');
        if(editBtn) editBtn.onclick = () => editLessonById(currentLessonId);

        const deleteBtn = document.getElementById('btn-delete');
        if(deleteBtn) deleteBtn.onclick = () => deleteLessonById(currentLessonId);

        sbSetLockedButton(document.getElementById('btn-read'), !sbCanPlay("read"));
        ...
        sbSetLockedButton(document.getElementById('btn-flash'), !sbCanPlay("flash"));
        sbSetLockedButton(document.getElementById('btn-sentence'), !sbCanPlay("sentence"));
        sbSetLockedButton(document.getElementById('btn-daily'), !sbCanPlay("daily"));

        const locksWrap = document.getElementById("sb-reading-game-locks");
        if(locksWrap && !sbIsParentLocked()){
          const games = [
            ...
            { id:"sentence", label:"🧠 Sentence Repeat" },
            { id:"daily", label:"💎 Daily Speaking" },
          ];
          ...
          locksWrap.innerHTML = `
            <div style="display:flex; align-items:center; justify-content:space-between; gap:10px; flex-wrap:wrap;">
              ...
              <div style="display:flex; gap:8px;">
                <button id="sb-reading-lock-all" style="background:#ff6b6b; padding:6px 10px;">Lock all</button>
                <button id="sb-reading-unlock-all" style="background:#2ecc71; padding:6px 10px;">Unlock all</button>
              </div>
            </div>
            <div style="margin-top:8px; display:flex; gap:10px; flex-wrap:wrap;" id="sb-reading-lock-grid"></div>
          `;

          const grid = document.getElementById("sb-reading-lock-grid");

          games.forEach(g => {
            const locked = sbIsReadingGameLocked(lessonKey, g.id);
            ...
            pill.querySelector("input").onchange = (e) => {
              sbSetReadingGameLocked(lessonKey, g.id, e.target.checked);
              openLessonHubById(currentLessonId);
            };
            grid.appendChild(pill);
          });

          document.getElementById("sb-reading-lock-all").onclick = () => {
            games.forEach(g => sbSetReadingGameLocked(lessonKey, g.id, true));
            openLessonHubById(currentLessonId);
          };

          document.getElementById("sb-reading-unlock-all").onclick = () => {
            games.forEach(g => sbSetReadingGameLocked(lessonKey, g.id, false));
            openLessonHubById(currentLessonId);
          };
        } else if(locksWrap){
          locksWrap.innerHTML = "";
  ...
  return {
    ...
    data_json: {
      ...
      window.sbBackToLesson = function(){
        if(currentLessonIndex >= 0 && lessons[currentLessonIndex]){
          openLessonHub(currentLessonIndex);
        } else {
          showPage('menu-page');
        }
      ...
      window.sbExportAll = sbExportAll;
      window.sbExportLessonById = sbExportLessonById;
...
function sbIsMarketplaceImportedLesson(lesson){
  return !!lesson?.data_json?.importedFromPackId;
}
...
function sbCanExportAllLessons(){
  return (lessons || []).every(sbCanExportLesson);
      ...
      async function sbBuildReadingExportPayload(lessonIds = null){
        await loadLessons();

        const picked = Array.isArray(lessonIds) && lessonIds.length
      ...
      async function sbExportLessonById(lessonId){
  ...
  const pack = await sbBuildReadingExportPayload([lessonId]);
  const safe = String(lesson.title || 'reading_lesson').replace(/[^a-z0-9-_]+/gi, '_');
  sbDownloadJson(`${safe}_reading_lesson.json`, pack);
  sbToast('⬇️ Lesson exported');
}

async function sbExportAll(){
  if(!sbCanExportAllLessons()){
    sbToast("One or more imported marketplace lessons cannot be exported.");
    return;
  }

  const pack = await sbBuildReadingExportPayload();
  sbDownloadJson('reading_lessons_export.json', pack);
...
async function sbAutoOpenMarketplaceReadingLesson(){
  ...
  if(!lesson){
    console.warn("Marketplace reading lesson not found for pack:", marketPackId);
    return false;
  }

  await openLessonHubById(lesson.id);
  return true;
}
...
window.sbExport = sbExport;
      ...
      (async function boot(){
        ...
        await sbConsumePendingImportReading();
        await loadLessons();
        showPage('menu-page');
        sbApplyParentMode();
