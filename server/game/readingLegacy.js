...
function triggerSentenceMic(){
  ...
  if (repeatIsListening && (Date.now() - repeatLastStartTs > 10000)) {
...
window.dailyHearAgain = function(){
  ...
  const list =
    window.currentLesson?.dailySpeaking ||
    window.currentLesson?.dailySpeakings ||
    window.currentLesson?.dailyPhrases ||
    [];

  let item = null;

  if (Array.isArray(list) && list.length) {
    if (typeof dailyIndex !== "number") window.dailyIndex = 0;
    item = list[dailyIndex] || list[0];
  }

  const text = typeof item === "string"
    ? item
    : (item?.text || item?.sentence || item?.phrase || "");

  if (!text) return;

  const textEl = document.getElementById("daily-speaking-text")
    || document.getElementById("daily-text")
    || document.getElementById("daily-display");
...
// const sentenceNext = document.getElementById("sentence-next-btn");
if (sentenceNext) {
  sentenceNext.onclick = window.nextSentence;
}

    /***********************
 * FIX: Speaking buttons + mic
 ***********************/
(function(){

  function safeCancelTTS(){
    try {
  ...
  function safeStartRecognition(recognition){
    if(!recognition) return;

    try {
      safeCancelTTS();

      // small delay so browser releases audio channel
      setTimeout(()=>{
        try {
          recognition.start();
        } catch(err){
  ...
  function sbBindSpeakingButtons(){
  const sentenceBtn  = document.getElementById("sentence-mic-btn");
  const dailyBtn     = document.getElementById("daily-mic-btn");
  const sentenceNext = document.getElementById("sentence-next-btn");
  const dailyNext    = document.getElementById("daily-next-btn");
  const dailyHearBtn = document.getElementById("daily-hear-again-btn");

  if(sentenceBtn){
    sentenceBtn.onclick = () => {
      if(typeof triggerSentenceMic === "function") triggerSentenceMic();
    };
  }

  if(dailyBtn){
    dailyBtn.onclick = () => {
      if(typeof triggerDailyMic === "function") triggerDailyMic();
    };
  }

  if(sentenceNext){
    sentenceNext.onclick = () => {
  ...
  if(dailyNext){
    dailyNext.onclick = () => {
      if(typeof window.nextDaily === "function") window.nextDaily();
    };
  }

  if(dailyHearBtn){
    dailyHearBtn.onclick = () => {
      if(typeof window.dailyHearAgain === "function") window.dailyHearAgain();
    };
  }
}
  ...
  //   if(sentenceBtn){
  //     sentenceBtn.onclick = () => {
  //       console.log("🎤 Sentence mic clicked");

  //       if(typeof triggerSentenceMic === "function"){
  //         triggerSentenceMic();
  //       } else if(window.repeatRecognition){
  //         safeStartRecognition(window.repeatRecognition);
  //       }
  //     };
  //   }
  ...
  //   if(dailyBtn){
  //     dailyBtn.onclick = () => {
  //       console.log("🎤 Daily mic clicked");

  //       if(typeof triggerDailyMic === "function"){
  //         triggerDailyMic();
  //       } else if(window.dailyRecognition){
  //         safeStartRecognition(window.dailyRecognition);
  //       }
  //     };
  //   }

  //   if(sentenceNext && typeof nextSentence === "function"){
  //     sentenceNext.onclick = nextSentence;
  //   }

  //   if(dailyNext && typeof nextDaily === "function"){
  //     dailyNext.onclick = nextDaily;
  //   }
  // }

  // run after DOM ready
  setTimeout(sbBindSpeakingButtons, 0);

  // ALSO re-bind every time page changes (IMPORTANT)
  const _oldShowPage = window.showPage;
  if(typeof _oldShowPage === "function"){
    window.showPage = function(page){
      _oldShowPage(page);

      if(page === "sentence" || page === "daily"){
        setTimeout(sbBindSpeakingButtons, 100);
      }
    };
  }
...
/* =========================
   Global Expose (inline onclick compatibility)
========================= */
...
window.nextSentence = function(){
  ...
  const list =
    window.currentLesson?.sentenceRepeats ||
    window.currentLesson?.sentences ||
    [];

  if (!Array.isArray(list) || !list.length) return;

  sentenceIndex++;
  if (sentenceIndex >= list.length) sentenceIndex = 0;

  const sentence = list[sentenceIndex];

  const textEl = document.getElementById("sentence-repeat-text")
    || document.getElementById("sentence-text")
    || document.getElementById("sentence-display");

  if (textEl) {
    textEl.textContent = typeof sentence === "string"
      ? sentence
  ...
  if (typeof speakText === "function") {
    ...
    const txt = typeof sentence === "string"
      ? sentence
      : (sentence?.text || sentence?.sentence || "");
    if (txt) speechSynthesis.speak(new SpeechSynthesisUtterance(txt));
  }
};
Object.defineProperty(window, 'currentLesson', {
  configurable: true,
  get(){ return currentLesson; },
  set(v){ currentLesson = v; }
});
Object.defineProperty(window, 'currentLessonIndex', {
  configurable: true,
  get(){ return currentLessonIndex; },
  set(v){ currentLessonIndex = v; }
});
