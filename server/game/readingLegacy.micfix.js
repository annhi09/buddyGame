...
function triggerSentenceMic(){
  ...
  repeatRecognition.onerror = (e)=>{
    repeatIsListening=false;
    resetBtn();
    const msg = (e && (e.error || e.message)) ? (e.error || e.message) : 'mic error';
    sbToast(`🎤 Sentence mic error: ${msg}`, 2200);
  };
  ...
  // Stop any TTS first; some browsers will not start recognition while speech is playing.
  try{ window.speechSynthesis.cancel(); }catch(e){}

  // Some mobile browsers are picky; defer start a bit after the tap / TTS cancel.
  setTimeout(()=>{
    try{
...
function triggerDailyMic(){
  ...
  repeatRecognition.onerror = (e)=>{
    ...
    resetBtn();
    const msg = (e && (e.error || e.message)) ? (e.error || e.message) : 'mic error';
    sbToast(`🎤 Daily mic error: ${msg}`, 2200);
  };
  repeatRecognition.onend   = ()=>{
    const elapsed = Date.now() - (sbListenStartedAt || Date.now());
    if(!sbGotResult && !sbAutoRestarted && elapsed < sbListenMinMs){
...
window.finishDailySpeaking = finishDailySpeaking;
