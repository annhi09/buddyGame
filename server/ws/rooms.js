...
export function broadcastExcept(room, skipWs, data) {
  const msg = JSON.stringify(data);
  for (const client of room.players.keys()) {
...
export function makeFallbackPlayerKey(msg) {
  ...
  const vehicle = normalizeVehicle(msg.vehicle);
  return `${role}:${name}:${team}:${character}:${vehicle}`;
}
...
export function startHostRecovery(room, hostPlayer) {
  ...
  room.offlinePlayers.set(hostPlayer.playerKey, {
    ...hostPlayer,
    vehicle: normalizeVehicle(hostPlayer.vehicle),
    connected: false,
    lastSeen: safeNow(),
  });
...
export function findReconnectCandidate(room, incoming) {
  ...
  for (const p of room.offlinePlayers.values()) {
    if (
      ...
      normalizeVehicle(p.vehicle) === normalizeVehicle(incoming.vehicle)
    ) {
      return p;
    }
  }
...
export function removePlayerSocket(ws) {
  ...
  room.offlinePlayers.set(player.playerKey, {
    ...player,
    vehicle: normalizeVehicle(player.vehicle),
    connected: false,
    lastSeen: safeNow(),
  });
